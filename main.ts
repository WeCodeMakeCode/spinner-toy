...
controller.left.onEvent(ControllerButtonEvent.Pressed, function () {
    if (!(b_same_speed)) {
        for (let value of spinner_list) {
            value.speed = speedLimitSlider.value
        }
        b_same_speed = true
    } else if (!(b_clockwise)) {
        for (let value of spinner_list) {
            value.direction = Direction.Clockwise
        }
        b_clockwise = true
    } else {
        for (let value of spinner_list) {
            value.direction = Direction.Counterclockwise
        }
        b_clockwise = false
    }
})
...
let spinner_list: spinner.Spinner[] = []
let b_clockwise = false
let b_same_speed = false
let b_follow = false
let cover: Sprite = null
let speedLimitSlider: Slider = null
speedLimitSlider = slider.create(5, 0, 20, 4, 80, Orientation.Vertical)
speedLimitSlider.left = 148
speedLimitSlider.selected = false
